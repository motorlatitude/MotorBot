...
impl Database {
  ...
  pub async fn open() -> Result<Self> {
    ...
    if !path.exists() {
      info!("Database file not found, creating new database at {}", &path.to_string_lossy());
      match Connection::open(&path) {
        Ok(conn) => {
          let mut db = Self { connection: Some(conn) };
          db.create_tables().await?;
          info!("Database created successfully at {}", &path.to_string_lossy());
          Ok(db)
        },
        Err(e) => {
          error!("Failed to create database: {:?}", e);
          Err(Error::ConnectionError { err: e })
        }
      }
    } else {
      let mut db = Self {
        connection: Some(Connection::open(path).map_err(|err| Error::ConnectionError { err })?)
      };
      ...
      if version == SCHEMA_VERSION {
        ...
        Err(Error::InvalidSchemaVersion { expected: SCHEMA_VERSION, found: version })
      }
    }
  }
  ...
  pub async fn user_score(&mut self, user_id: &u64) -> Result<UserScore> {
    match &mut self.connection {
      Some(connection) => {
        const Q_USER_SCORE: &str = "SELECT user_id, score FROM user_scores WHERE user_id = ?1";
        let mut stmt = connection.prepare(Q_USER_SCORE)
          .map_err(|e| Error::with_sql(e, Q_USER_SCORE))?;
  ...
  pub async fn set_user_score(&mut self, user_id: &u64, score: i32) -> Result<UserScore> {
    match &mut self.connection {
      Some(connection) => {
        const Q_SET_USER_SCORE: &str = "INSERT INTO user_scores (user_id, score) VALUES (?1, ?2)
          ON CONFLICT(user_id) DO UPDATE SET score = excluded.score";
        connection.execute(Q_SET_USER_SCORE, params![user_id.to_string(), score])
          .map_err(|e| Error::with_sql(e, Q_SET_USER_SCORE))?;
        Ok(UserScore {
          user_id: *user_id,
          score,
        })
  ...
  pub async fn game_news(&mut self, game_id: &str) -> Result<Vec<String>> {
    match &mut self.connection {
      Some(connection) => {
        ...
        let mut stmt = connection.prepare(Q_GAME_NEWS)
          .map_err(|e| Error::with_sql(e, Q_GAME_NEWS))?;
  ...
  /// * `Err(Error)` - If there is an error during the database operation, it
  /// returns a [Error] variant with details about the failure.
  pub async fn add_game(&mut self, game_id: &str, guild: u64, platform: Platform, name: &str, thumbnail: &str, color: &str) -> Result<()> {
    match &mut self.connection {
      Some(connection) => {
        const Q_ADD_GAME: &str = "INSERT INTO games (id, guild, platform, name, thumbnail, color) VALUES (?1, ?2, ?3, ?4, ?5, ?6)";
        connection.execute(Q_ADD_GAME, params![game_id, guild.to_string(), platform.to_string(), name, thumbnail, color])
            .map_err(|e| Error::with_sql(e, Q_ADD_GAME))?;
        Ok(())
      },
      None => return Err(Error::InvalidConnection),
    }
  ...
  /// * `Err(Error)` - If there is an error during the database operation, it
  /// returns a [Error] variant with details about the failure.
  pub async fn remove_game(&mut self, game_id: &str, guild: u64) -> Result<()> {
    match &mut self.connection {
      Some(connection) => {
        const Q_REMOVE_GAME: &str = "DELETE FROM games WHERE id = ?1 AND guild = ?2";
        connection.execute(Q_REMOVE_GAME, params![game_id, guild.to_string()])
            .map_err(|e| Error::with_sql(e, Q_REMOVE_GAME))?;
        Ok(())
      },
      None => return Err(Error::InvalidConnection),
    }
  ...
  pub async fn game_details(&mut self, game_id: &str) -> Result<GameData> {
    match &mut self.connection {
      Some(connection) => {
        ...
        let mut stmt = connection.prepare(Q_GAME_DETAILS)
  ...
  /// * `Err(Error)` - If there is an error during the database query, it
  /// returns a [Error] variant with details about the failure.
  pub async fn game_ids_for_guild(&mut self, guild: u64) -> Result<Vec<String>> {
    match &mut self.connection {
      Some(connection) => {
        const Q_GAME_IDS_FOR_GUILD: &str = "SELECT id FROM games WHERE guild = ?1";
        let mut stmt = connection.prepare(Q_GAME_IDS_FOR_GUILD)
            .map_err(|e| Error::with_sql(e, Q_GAME_IDS_FOR_GUILD))?;
  ...
  /// * `Err(Error)` - If there is an error during the database operation, it
  /// returns a [Error] variant with details about the failure.
  pub async fn add_news_item(&mut self, game_id: &str, news_id: &str) -> Result<()> {
    match &mut self.connection {
      Some(connection) => {
        const Q_ADD_NEWS_ITEM: &str = "INSERT INTO game_news (game_id, news_id) VALUES (?1, ?2)";
        connection.execute(Q_ADD_NEWS_ITEM, params![game_id, news_id])
            .map_err(|e| Error::with_sql(e, Q_ADD_NEWS_ITEM))?;

        // Remove old news items if there are more than 5
        const Q_COUNT_NEWS_ITEMS: &str = "SELECT COUNT(*) FROM game_news WHERE game_id = ?1";
        let mut stmt = connection.prepare(Q_COUNT_NEWS_ITEMS)
            .map_err(|e| Error::with_sql(e, Q_COUNT_NEWS_ITEMS))?;
        let count: i32 = stmt.query_row(params![game_id], |row| row.get(0))
            .map_err(|e| Error::with_sql(e, Q_COUNT_NEWS_ITEMS))?;
        ...
        if count > 5 {
            ...
            connection.execute(Q_DELETE_OLD_NEWS, params![game_id, count - 5])
                .map_err(|e| Error::with_sql(e, Q_DELETE_OLD_NEWS))?;
        }
        Ok(())
      },
      None => return Err(Error::InvalidConnection),
  ...
  /// * `Err(Error)` - If there is an error during the database operation, it
  /// returns a [Error] variant with details about the failure.
  pub async fn set_guild_config(&mut self, guild_id: u64, guild_config: GuildConfig) -> Result<()> {
    match &mut self.connection {
      Some(connection) => {
        ...
        connection.execute(Q_SET_GUILD_CONFIG, params![guild_id.to_string(), config_key, config_value])
          .map_err(|e| Error::with_sql(e, Q_SET_GUILD_CONFIG))?;
        Ok(())
      },
      None => return Err(Error::InvalidConnection),
    }
  ...
  /// * `Err(Error)` - If there is an error during the database query, it
  /// returns a [Error] variant with details about the failure.
  pub async fn get_guild_config(&mut self, guild_id: u64, guild_config_key: GuildConfigKey) -> Result<Option<GuildConfig>> {
    match &mut self.connection {
      Some(connection) => {
        const Q_GET_GUILD_CONFIG: &str = "SELECT config_value FROM guild_config WHERE guild_id = ?1 AND config_key = ?2";
        let mut stmt = connection.prepare(Q_GET_GUILD_CONFIG)
          .map_err(|e| Error::with_sql(e, Q_GET_GUILD_CONFIG))?;
        let mut rows = stmt.query(params![guild_id.to_string(), guild_config_key.as_ref()])
          .map_err(|e| Error::with_sql(e, Q_GET_GUILD_CONFIG))?;

        if let Some(row) = rows.next()? {
          let config_value: String = row.get(0)?;
          Ok(Some(GuildConfig::from((guild_config_key, config_value))))
        } else {
  ...
  pub async fn close(&mut self) -> Result<()> {
    if let Some(conn) = self.connection.take() {
      conn.close().map_err(|(_conn, err)| Error::ConnectionError { err })?;
    ...
    match &mut self.connection {
      Some(connection) => {
        ...
        connection.pragma_update(None, "user_version", &SCHEMA_VERSION)
            .map_err(|e| Error::with_sql(e, "Failed to set schema version"))?;

        // Create user_scores table
        const Q_CREATE_USER_SCORES_TABLE: &str = "CREATE TABLE IF NOT EXISTS user_scores (
        ...
        connection.execute(
          ...
          []
        ).map_err(|e| Error::with_sql(e, Q_CREATE_USER_SCORES_TABLE))?;

        // Create platforms table
        const Q_CREATE_PLATFORMS_TABLE: &str = "CREATE TABLE IF NOT EXISTS platforms (
            id TEXT PRIMARY KEY,
            name TEXT NOT NULL
        )";
        connection.execute(
            ...
            [],
        ).map_err(|e| Error::with_sql(e, Q_CREATE_PLATFORMS_TABLE))?;
        const Q_INSERT_PLATFORMS: &str = "INSERT OR IGNORE INTO platforms (id, name) VALUES
            ('steam', 'Steam'),
            ('riot', 'Riot Games')";
        connection.execute(
            Q_INSERT_PLATFORMS,
            []
        ).map_err(|e| Error::with_sql(e, Q_INSERT_PLATFORMS))?;

        // Create games table
        const Q_CREATE_GAMES_TABLE: &str = "CREATE TABLE IF NOT EXISTS games (
            id TEXT NOT NULL,
            guild TEXT NOT NULL,
        ...
        connection.execute(
            ...
            [],
        ).map_err(|e| Error::with_sql(e, Q_CREATE_GAMES_TABLE))?;
        ...
        connection.execute(
            ...
            [],
        ).map_err(|e| Error::with_sql(e, Q_CREATE_GAME_NEWS_TABLE))?;

        // Create guild_config table
        const Q_CREATE_GUILD_CONFIG_TABLE: &str = "CREATE TABLE IF NOT EXISTS guild_config (
        ...
        connection.execute(
            ...
            [],
        ).map_err(|e| Error::with_sql(e, Q_CREATE_GUILD_CONFIG_TABLE))?;
  ...
  async fn schema_version(&mut self) -> Result<u8> {
    match &mut self.connection {
      Some(connection) => {
        ...
        let mut stmt = connection.prepare(Q_GET_SCHEMA_VERSION)
            .map_err(|e| Error::with_sql(e, Q_GET_SCHEMA_VERSION))?;
        let version: u8 = stmt.query_row([], |row| row.get(0))
            .map_err(|e| Error::with_sql(e, Q_GET_SCHEMA_VERSION))?;
